...
function OwnerControls({ availability, onAvailabilityChange, onMenuChange }) {
  ...
  const [menuItems, setMenuItems] = useState(() => readStoredValue(MENU_KEY, defaultMenuItems));
  ...
  const updateMenuItem = (id, field, value) => {
    saveMenu(menuItems.map((item) => (item.id === id ? { ...item, [field]: value } : item)));
  };

  const addMenuItem = () => {
    if (!newItem.name.trim() || !newItem.price.trim()) {
      alert("Please add at least dish name and price.");
      return;
    }
        ...
        <div className="mt-4 max-h-[78vh] w-[min(92vw,520px)] overflow-auto rounded-2xl border border-amber-400/20 bg-[#080808] p-5 shadow-2xl">
          ...
          <div className="mt-7 rounded-2xl border border-white/10 bg-white/5 p-4">
            ...
            <div className="mt-4 grid gap-3">
              ...
              <input value={newItem.price} onChange={(e) => setNewItem({ ...newItem, price: e.target.value })} placeholder="Price, example: Rs. 180" className="owner-input" />
